...
void sortDatabySelection(struct Q4Struct array[], int size)
{
    ...
    while(!done)
    {
        ...
        { 
            curr = array[i].intData;
            for (j = i; j < size; j++){
                next = array[j].intData;
                if (curr > next){
                    temp.intData = array[i].intData;
                    temp.charData = array[i].charData;

                    array[i].intData = array[j].intData;
                    array[i].charData = array[j].charData;

                    array[j].intData = temp.intData;
                    array[j].charData = temp.charData;
                    done = 0;
                }
            }            
        }
