...
Matrix Matrix::copy(){
	...
	//Matrix copy = Matrix(); //Need to set size.
	int rowsize = getsizeofrows();
	int colsize = getsizeofcols();
	Matrix copy = Matrix(rowsize, colsize);
	for(int i = 0; i < rowsize; i++){
		for(int j = 0; j < colsize; j++){
			//cout << "\nDebug: " << matrixData[j][i] << "\n"; //Should be [i][j]
...
void Matrix::addTo( Matrix &m ){
	...
	int rowsize = getsizeofrows();
	int colsize = getsizeofcols();
	if (m.getsizeofcols() == colsize && m.getsizeofrows() == rowsize){
		for(int i = 0; i < rowsize; i++){
			for(int j = 0; j < colsize; j++){
...
Matrix Matrix::subMatrix(int i, int j){
	...
	if (getsizeofrows() <= i || getsizeofcols() <= j){
		throw std::out_of_range("Submatrix not defined.");
	}
	else{
		Matrix subMat = Matrix(i+1, j+1);
		for (int r = 0; r <= i; r++){
			for (int c = 0; c <= j; c++){
				subMat.setElement(getElement(r,c), r, c);
			}
		}
		return subMat;
	}
